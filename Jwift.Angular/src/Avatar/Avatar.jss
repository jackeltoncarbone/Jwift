...
// The house glass rim: a constant hairline whose brightness alone follows the light, screened onto the
// disc, so the ring is the disc's own color at a higher value all the way round.
//
// The rim still shows through a photo: it draws at BorderLayer 10, above the content, so the bezel
// survives `Overflow: Hidden` clipping the image to the disc. Jack on that: "the border has a layer just
// like anything else -- if it's over the contents children, it uses that."
