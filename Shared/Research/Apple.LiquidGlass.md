...
| Identity in a browser tab: Apple's web apps lead the sidebar with the wordmark (music.apple.com, apps.apple.com) | the HOME surface's header carries the lifted wordmark where every other surface carries its own name; the sidebar still starts at Home with no wordmark | settled 2026-09-22, Jack's call. The native rule ("the top of a native sidebar is a control row, never identity") still holds for the sidebar. The header is the Arcade case instead: Apple's own hero tab reads "&#63743; Arcade" because the tab IS the brand, and Home is our front door. Identity appears exactly once, and the hero carries no mark at all |
