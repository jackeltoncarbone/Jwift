...
| bar | the X leading and the checkmark trailing (`[confirmable]`, `(confirm)`), each the 44 pt bar button (`<glass-button size="bar">`) at `@JwiftSheetBarInset` (22 pt; Apple's is 16 in a 38 pt corner), so its center is the corner's; untitled sheets keep the X; the title 17 pt semibold on the buttons' center line; `[navigationDepth]` above 0 turns the leading X into Back (`(back)`) for a screen pushed inside the sheet, as a navigation stack in a sheet does (swipe, dim and Escape still dismiss); `[barAction]` puts one glyph button trailing (`(barActionPress)`), Notes' and Mail's compose | section 3 [C][I] | `Jwift_SheetBar`, `GlassButton.jss` `Jwift_GlassBtnBar_*` |
...
| material | Liquid Glass while at most half way to full height, the opaque `@Sheet` above; a form sheet floats clear of every edge, so it never goes full height and is regular glass at every size (iPad's glass has no `sheet` subvariant, so it keeps the outer refraction and edge bleed) | `sub_1891102E0`, `sub_18910A6A0` [C] | `Jwift_SheetGlass`, `Jwift_SheetGlass_Form`, `Jwift_SheetOpaque` |
...
| footer | the body's last child is its footer, and an action there shares the bottom corners' center: `@JwiftSheetContentInset` (44 - 24 = 20 pt for a 48 pt capsule) beside it and the same below it, grown by what the bottom corner has over the top one (52 at the large detent) and never inside the home indicator's safe area once the sheet reaches the edge. Content never adds bottom padding of its own; a footer that bleeds to the card's edges pads its bottom by `@JwiftSheetFooterInset` | "a button that is positioned at the bottom of a sheet should share the same corner center with the corners of the sheet" [D] (323) | `FooterInset`, `BodyLayout` |
...
| Share (who is on a show) | `App/Drill/Collaborators/DrillShareSheet.ts` | large, pushed screens with Back | dimmed | Notes / Freeform "Manage Shared Note" |
| Intelligence (the assistant) | `Intelligence/AssistantSheet.ts` | medium, large (grabber) | undimmed at medium | Maps / Notes nonmodal sheets: the work stays live behind it |
