...
- Frost, the recipe's blur class (called a size class before it was read): Automatic ramps the blur 1.33 to 4 pt; Reduced is a constant 0.667 pt at backdropScale 0.5; None has no blur. [C] `GlassMaterialProvider`. It is not a size: UIKit sets it from the scroll pocket the glass sits in (3.2, Frost). [C]
...
The recipe's ramp, read again in iOS 26.1 DesignLibrary (`sub_18AF952E4`, DesignLibrary_13.mm 6640 to 6760): frost 0 sets min 1.33333333 and max 4.0, stored with the knots 48.0 and 160.0 at +360 to +384 of the spec; frost 1 sets 0.666666667 (0 when `sub_18AFA4B74` is true) with backdropScale 0.5; frost 2 sets 0 and keeps backdropScale 0.25. [C] How the backdrop mips are made before that LOD read is not in the restore: walle found a radius-dependent downsample blur (`DownsampleBlurUniforms`, five binary16 weights) whose inputs it could not observe. [C] structure, [I] its law.
...
Other captures, same method (`Evidence.md` 1b): the App Store tab bar (60 pt, 3x) reads σ 5.0 to 5.3 px behind it, the law's 4.9. The Photos segmented bar (45 pt) reads 2.5 to 2.7 px, below Automatic's 4.5 px: None's 2.48 px (0.62 of a 4 px texel), which is what the source gives glass in a blurring pocket (Frost, below). [I] Find My's sheet (Newsroom render) reads 44 to 166 px at 3.65 px per pt, 12 to 45 pt: a sheet's material is not the 4 pt glass body. [I]

**Frost: which glass takes which class.** [C] unless marked.
- The byte the recipe branches on (`*(a1 + 14)` in `sub_18AF952E4`) is `GlassMaterialProvider.Configuration.frost`, a `Frost.Base { automatic, reduced, none }`: DesignLibrary's Swift metadata lists the configuration as base, frost, subvariant, size, options, and the three-case enum is the only field with the recipe's three branches (the byte offset itself is [I]).
- UIKit's `_Glass._GlassVariant` is `{ config, frost, size, controlTint }`; `Frost.Base` automatic 0, reduced 1, none 2 (static getters 0x188F66680, 0x188F66668, 0x188F66674 in the firmware); `Frost.pocketDefault` is automatic. `_UIViewGlass.size` is a different field (`_GlassVariant.Size { dynamic, small, medium, large }`), and every UIKit caller of `initWithVariant:size:smoothness:` and `…subdued:` passes size 0 (x3 = 0 at 0x188C172A4, 0x189C66D74, 0x189C67018).
- Frost travels as a trait, `GlassFrostTrait` (an NSInteger trait; glass re-resolves on it, UIKitCore_22.mm `sub_188D62FD0`; SwiftUI reads it into its environment, UIKitCore_35.mm `sub_188EB3AA8`).
- The trait is written by a scroll pocket container (`_UIScrollPocketContainerInteraction`'s implementation, UIKitCore_72.mm and UIKitCore_06.mm): set to the pocket's frost when it has one, removed (Automatic) when not. Per edge, UIKit folds its pocket elements (UIKitCore_46.mm `sub_188FAAF08`): any Reduced wins, else any None gives None, else Automatic; an element reports its `ScrollEdgeEffectView`'s frost while that view is in a window, Automatic otherwise (UIKitCore_53.mm `sub_18904AA54`).
- A pocket's own frost (UIKitCore_53.mm, the layout pass near line 8793): None when its blur alpha is 0 or when its `PocketBlur.blurEnabled` is on and the edge effect is showing; Reduced otherwise. `blurEnabled` is on for the hard style unless `_prefersSolidColorHardPocket` (off under Reduce Transparency); for the soft style it follows a flag not recovered. A fresh pocket starts at None.
- The containers: the phone tab bar controller's bottom edge (`_UITabBarControllerVisualStyle_Phone`, `initWithScrollView:0 edge:4`), and the selectors `navigationBarPocketContainerInteraction`, `bottomTabBarPocketContainerInteraction`, `composeInputAccessoryPocketContainerInteraction`. A menu platter lives outside them (Menus, above), so it is Automatic.
- On Apple's frames [I]: the Photos segmented bar in the bottom pocket reads None (2.5 to 2.7 px, None's 2.48); the App Store tab bar in the same kind of pocket reads Automatic (5.0 to 5.3 px, the law's 4.9). Why the tab bar keeps Automatic is not read; the port follows the frames.
...
- Ours, not Apple's: `FlexStretch: Auto | <number>` multiplies the translation stretch and the acceleration squash about the lift (1 is the law above, 0 none), scaling `moveScalePts` and the min / max range around 1 together; it and `FlexLift`, `FlexBigGlow`, `FlexLittleGlow` spring like any JSS number (Jaui Core/Glass.Jss.md 3a). UIKit drops these elastic properties under Reduce Motion; Show Studio does not consume Reduce Motion (Jack, 2026-09-18).
